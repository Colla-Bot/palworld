...
constexpr const char *defaultName        = "Link2";
...
MumbleStringWrapper mumble_getAuthor() {
	static const char *author = "Mumble Developers; Argo";
...
bool mumble_fetchPositionalData(float *avatarPos, float *avatarDir, float *avatarAxis, float *cameraPos,
	...
	if (lm.uiTick != last_tick) {
		last_tick      = lm.uiTick;
		last_tick_time = getTimeSinceEpoch();
	} else if ((getTimeSinceEpoch() - last_tick_time) > 60000) { // jebibot: increase timeout
		return false;
	}
